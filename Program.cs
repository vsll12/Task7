...
//Main de verilen sertlere uygun olaraq melumatlari cixarmaq lazimdir.
...
class Program
{
    ...
    public static void Main()
    {
        ...
        //    1) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        //var result = debtors.Where(d => DateTime.Now.Year - d.BirthDay.Year > 26 && DateTime.Now.Year - d.BirthDay.Year < 36);

        //foreach (var item in result)
        //{
        //    Console.WriteLine($"{item.FullName}  {item.BirthDay.Year}");
        //}

        //    2) Borcu 5000 - den cox olmayan borclularic cixartmag

        //var result = debtors.Where(d => d.Debt < 5000);
        ...
        //var result = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com"));
        ...
        //    3) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        //var result = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2).ToList();
        ...
        //var results = debtors.Where(d => d.BirthDay.Month >= 12 && d.BirthDay.Month < 3);
        ...
        //var result = debtors.Where(d => d.Debt > debtors.Average(d => d.Debt));
        ...
        //    6) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        //var result = debtors.Where(d => !d.Phone.Contains('8')).ToList();

        //foreach (var debtor in result)
        //{
        //    Console.WriteLine($"{debtor.FullName.Split('.')[1]} {DateTime.Now.Year -  debtor.BirthDay.Year}  {debtor.Debt}");
        //}
        ...
        //var result = debtors.Where(d => d.FullName.Split('.')[0].Count(ch =>);
        ...
        //var result = debtors.OrderByDescending(d => d.Debt).ToList();
        ...
        //decimal sum = debtors.Sum(d => d.Debt);

        //Console.WriteLine(sum);

        //    11)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        //var result = debtors.Where(d => d.BirthDay.Year - 1939 <= 0);
        ...
        //var result = debtors.Where(d => d.Debt - (500 * (Math.Abs(d.BirthDay.Month - DateTime.Now.Month))) <= 0).ToList();
        ...
        //    14)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq

        //var result = debtors.Where(d => d.FullName.Contains('s') && d.FullName.Contains('m') && d.FullName.Contains('i') && d.FullName.Contains('l') && d.FullName.Contains('e'));
